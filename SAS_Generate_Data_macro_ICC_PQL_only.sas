...
%macro pql;
ods select none;
ods output parameterestimates = pqlestimates covparms = cp;
ods trace on;
title "pql";
proc glimmix
	data = sim
...
data results; 
set pqlestimates (in = pql) cp (in = pql);
...
%runsim(							/* n is number of clusters, p is people per cluster */
	dsn = 2, p = 100, n = 50, 		/* p used to be about 75, n about 40. trying larger number to get better convergence */
	beta0 = %str(-3.891820298),
/*  beta0 = %str(-11.51291546,-9.210240367,-6.906754779,-5.293304825,-4.59511985,-3.891820298,-3.47609869,-2.944438979,-2.197224577,-1.386294361,-0.8472978604,-0.4054651081),*/
	beta1 = %str(-0.1053605157, 0.2851789422)
...
/*proc means data = results_or (where = (effect = "x1"));
class beta0 beta1;
var Odds_Ratio;
run;*/
...
data results_icc;
set results (where = (covparm = "Intercept"));
icc = (estimate) / (estimate + 3.29);
run;

proc means data = results_icc (where = (covparm = "Intercept"));
class beta0 beta1;
var icc;
run;
